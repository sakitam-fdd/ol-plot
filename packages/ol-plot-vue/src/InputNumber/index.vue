<template>
  <div ref="root" class="ol-plot-vue-input-number"
       :class="[
...
export default defineComponent({
  ...
  setup(props, { slots, attrs, expose, emit }) {
    ...
    const minDisabled = computed(
        () => _decrease(props.value, props.step) < props.min,
    )
    const maxDisabled = computed(
        () => _increase(props.value, props.step) > props.max,
    )
    const precision = computed(() => {
      return Math.max(getPrecision(props.value), getPrecision(props.step));
    });
    ...
    const _increase = (val: any, step: number) => {
      ...
      const precisionFactor = Math.pow(10, precision.value);
