                    ...
                    <span
                      :style="{
                        background: `url(${getImage(
                          tool.src + (tool.mouseover || selected === tool.alias ? '-hover' : ''),
                        )}) no-repeat`,
                      }"
                    />
                    <span>{{ tool.name }}</span>
    ...
    setup(props, { emit }) {
        ...
        if (overlay) {
          ...
          if (_style) {
            ...
            if (_style.border) {
              ...
              state.textAreaBorderColor = _style.border.slice(
                _style.border.indexOf('rgb('),
                _style.border.indexOf(')') + 1,
              );
            }
            if (_style.background) {
              state.textAreaBackgroundColor = _style.background.slice(
                _style.background.indexOf('rgb('),
                _style.background.indexOf(')') + 1,
              );
            }
          }
        }
      ...
      function init() {
        ...
        if (!plot.value && map) {
          ...
          plot.value.plotDraw.drawLayer.setStyle(
            new Plot.StyleFactory({
              fill: {
                fillColor: '#8BA2E4',
              },
              stroke: {
