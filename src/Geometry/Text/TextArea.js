...
class TextArea extends mixin(Observable) {
  ...
  _handleResolutionChange (event) {
    if (this.options['zoomWithView'] && this.feature && this.feature instanceof $Feature) {
    ...
    if (!this.isMoveing) {
      this.getImage(event.target).then(icon => {
        ...
        this.feature = new $Feature({
          geometry: new $Point(JSON.parse(coordinates))
        })
        let _labelFeature = new $Feature({
        ...
        _labelFeature.setStyle(new $Style({
          fill: new $Fill({
