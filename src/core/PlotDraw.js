...
import PlotText from '../Geometry/Text/PlotText'
...
class PlotDraw extends mixin(PlotFactory, Observable, olLayerLayerUtils) {
  ...
  addWindowEventListener () {
    document.querySelector('.ol-overlaycontainer').addEventListener('click', event => {
      let ev = event || window.event
      let target = ev.target || ev.srcElement
      if (target.nodeName.toLowerCase() === 'div') {
        if (this.textAreas && this.textAreas.length > 0) {
          this.textAreas.every(item => {
            if (item['uuid'] === target.id) {
              this.dispatch('active_textArea', item)
            }
          })
        }
      }
